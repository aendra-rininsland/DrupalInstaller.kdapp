// All code brutally copied from Gökmen's brilliant WordPress installer.
...
WpApp = (function(_super) {
  ...
  function WpApp() {
    ...
    this.dashboardTabs = new KDTabView({
      ...
      cssClass: "drupal-installer-tabs"
    });
    ...
    this.dashboardTabs.on("PaneDidShow", function(pane) {
      if (pane.name === "dashboard") {
        ...
        return _this.buttonGroup.buttonReceivedClick(_this.buttonGroup.buttons["Install a new Drupal site"]);
      }
    });
  }
  ...
  WpApp.prototype.viewAppended = function() {
    ...
    installPane.on("DrupalInstalled", function(formData) {
      var domain, path;
...
var appStorage, checkPath, fc, installDrupal, kc, nickname, parseOutput, prepareDb, tc;
...
installDrupal = function(formData, dbinfo, callback) {
  ...
  if (db) {
    ...
    //commands.push("sed -i '' 's/database_name_here/" + dbinfo.dbName + "/g' '" + tmpAppDir + "/drupal/sites/default/settings.php'");
    //commands.push("sed -i '' 's/username_here/" + dbinfo.dbUser + "/g' '" + tmpAppDir + "/drupal/sites/default/settings.php'");
    //commands.push("sed -i '' 's/password_here/" + dbinfo.dbPass + "/g' '" + tmpAppDir + "/drupal/sites/default/settings.php'");
    //commands.push("sed -i '' 's/localhost/" + dbinfo.dbHost + "/g' '" + tmpAppDir + "/wordpress/sites/default/settings.php'");
...
DashboardPane = (function(_super) {
  ...
  function DashboardPane() {
    ...
    this.notice = new KDCustomHTMLView({
      tagName: "p",
      cssClass: "why-u-no",
      partial: "No sites installed yet."
    });
    this.notice.hide();
...
InstallPane = (function(_super) {
    ...
    this.form = new KDFormViewWithFields({
      ...
      fields: {
        name: {
          ...
          name: "name",
          placeholder: "type a name for your site...",
  ...
  InstallPane.prototype.submit = function(formData) {
    ...
    successCb = function(dbinfo) {
      return installDrupal(formData, dbinfo, function(path, timestamp) {
        _this.emit("Drupal Installed", formData);
        return _this.form.buttons["Install Drupal"].hideLoader();
      });
    };
